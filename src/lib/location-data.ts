// Static location reference data for the line-item location pickers.

export interface CountryOption {
  /** ISO 3166-1 alpha-2 code, uppercase — used for Nominatim countrycodes param. */
  code: string;
  name: string;
}

export type ComboboxItem = CountryOption | { divider: true };

export function isDivider(item: ComboboxItem): item is { divider: true } {
...
// Full ISO 3166-1 list, alphabetical, excluding the two pinned entries.
...
/** Country list for the combobox: pinned US + UK, divider, then alphabetical. */
export const COUNTRY_ITEMS: ComboboxItem[] = [
  ...PINNED,
  { divider: true },
  ...ALL_COUNTRIES,
];

/** All options flattened (no dividers) for code ↔ name lookups. */
const ALL_OPTIONS: CountryOption[] = [...PINNED, ...ALL_COUNTRIES];

/** Resolve a country name to its ISO alpha-2 code (case-insensitive). */
export function countryNameToCode(name: string): string | undefined {
  const n = name.trim().toLowerCase();
  // Handle legacy "USA" entries from seed data.
  if (n === "usa") return "US";
  return ALL_OPTIONS.find((c) => c.name.toLowerCase() === n)?.code;
}
...
export const UK_COUNTRIES: string[] = [
